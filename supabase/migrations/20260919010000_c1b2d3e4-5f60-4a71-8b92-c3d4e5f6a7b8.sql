-- Esta migração duplicava por engano a criação de credit_cards/
-- credit_card_transactions já feita em
-- 20260918115828_ac22b75e-9443-4e63-9fd6-a017e51c196b.sql, o que quebra
-- "aplicar migrações" com "relation already exists" quando as duas rodam em
-- sequência. Envolvida num guard: só executa se credit_cards ainda não
-- existir (banco novo / a primeira migração ainda não ter rodado).
do $$
begin
  if not exists (
    select 1 from pg_tables where schemaname = 'public' and tablename = 'credit_cards'
  ) then

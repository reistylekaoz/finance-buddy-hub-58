-- Integração bancária automática via Pluggy (Open Finance): guarda uma
-- conexão por instituição ligada pelo usuário, e liga cada conta/cartão
-- local à conta correspondente na Pluggy para sincronizar sem duplicar.
...
CREATE UNIQUE INDEX accounts_pluggy_account_id_idx ON public.accounts(pluggy_account_id) WHERE pluggy_account_id IS NOT NULL;

ALTER TABLE public.credit_cards
  ADD COLUMN IF NOT EXISTS bank_connection_id uuid REFERENCES public.bank_connections(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS pluggy_account_id text;
CREATE UNIQUE INDEX credit_cards_pluggy_account_id_idx ON public.credit_cards(pluggy_account_id) WHERE pluggy_account_id IS NOT NULL;

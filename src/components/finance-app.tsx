...
function Dashboard({ totals, chartDataByCurrency, transactions, accounts, categoryPath, centerName, centerSummary }: any) {
  return <div className="space-y-4">
    ...
    <section className="rounded-lg border border-border bg-card"><div className="border-b border-border px-5 py-4"><h2 className="font-semibold">Movimentações recentes</h2></div><TransactionRows transactions={transactions.slice(0, 6)} accounts={accounts} categoryPath={categoryPath} centerName={centerName} /></section>
...
function Accounts({ accounts, onAdd, rateDate }: { accounts: (Account & { balance: number; currency: string; balanceBRL: number })[]; onAdd: () => void; rateDate: string }) { if (!accounts.length) return <Empty title="Comece pelas suas contas" text="Cadastre bancos, carteiras e investimentos." onAdd={onAdd}/>; return <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{accounts.map(a => <div key={a.id} className="rounded-lg border border-border bg-card p-5"><div className="flex items-start justify-between"><div className="grid size-10 place-items-center rounded-md bg-primary-soft text-primary"><Landmark /></div><span className="rounded-full bg-muted px-2 py-1 text-xs text-muted-foreground">{a.is_active ? "Ativa" : "Inativa"}</span></div><h2 className="mt-5 font-semibold">{a.name}</h2><p className="text-sm text-muted-foreground">{a.institution || "Conta pessoal"}</p><p className="mt-4 font-mono text-2xl tabular-nums">{formatCurrency(a.balance, a.currency)}</p>{a.currency !== "BRL" && <p className="mt-1 text-xs text-muted-foreground">{money.format(a.balanceBRL)} pela cotação{rateDate ? ` de ${dateFmt.format(new Date(`${rateDate}T12:00:00`))}` : " do dia anterior"}</p>}</div>)}</div>; }
function TransactionRows({ transactions, accounts, categoryPath, centerName }: { transactions: Transaction[]; accounts: Account[]; categoryPath: (id:string|null)=>string; centerName: (id:string|null)=>string }) { return <div className="divide-y divide-border">{transactions.map(tx => { const positive=tx.transaction_type==="income"; const account = accounts.find(a=>a.id===tx.account_id); const currency = account?.currency || "BRL"; return <div key={tx.id} className="grid grid-cols-[1fr_auto] items-center gap-4 px-5 py-3 transition-colors hover:bg-muted/40 sm:grid-cols-[110px_1fr_1fr_auto]"><span className="hidden text-xs text-muted-foreground sm:block">{dateFmt.format(new Date(`${tx.transaction_date}T12:00:00`))}</span><div><p className="text-sm font-medium">{tx.description}</p><p className="text-xs text-muted-foreground sm:hidden">{dateFmt.format(new Date(`${tx.transaction_date}T12:00:00`))}</p></div><div className="hidden text-xs text-muted-foreground sm:block">{tx.transaction_type === "transfer" ? "Transferência" : categoryPath(tx.category_id)} · {account?.name}{tx.transaction_type !== "transfer" && <span className="ml-1 rounded-full bg-primary-soft px-2 py-0.5 text-primary">{centerName(tx.cost_center_id)}</span>}</div><span className={cn("font-mono text-sm tabular-nums", positive ? "text-income" : tx.transaction_type === "expense" ? "text-expense" : "text-foreground")}>{positive ? "+" : tx.transaction_type === "expense" ? "−" : ""}{formatCurrency(Number(tx.amount), currency)}</span></div>})}{!transactions.length && <p className="p-8 text-center text-sm text-muted-foreground">Nenhum lançamento neste período.</p>}</div>; }
function Transactions({ transactions, accounts, categoryPath, centerName, onAdd }: any) { if (!transactions.length) return <Empty title="Registre a primeira movimentação" text="Adicione receitas, despesas ou transferências." onAdd={onAdd}/>; return <section className="rounded-lg border border-border bg-card"><TransactionRows transactions={transactions} accounts={accounts} categoryPath={categoryPath} centerName={centerName}/></section>; }

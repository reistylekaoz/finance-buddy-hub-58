...
import { ArrowRightLeft, Building2, FileUp, ChevronRight, CircleDollarSign, Landmark, LayoutDashboard, LogOut, Menu, Pencil, Plus, Trash2, Shapes, TrendingUp, WalletCards, X } from "lucide-react";
...
export function FinanceApp() {
  ...
  const currencyOf = (accountId: string | null) => (accountId ? accountCurrency[accountId] ?? "BRL" : "BRL");
  const sortCurrencies = (keys: string[]) => [...keys].sort((a, b) => (a === "BRL" ? -1 : b === "BRL" ? 1 : a.localeCompare(b)));
  function sumByCurrency(items: Transaction[]): Record<string, number> {
    const out: Record<string, number> = {};
    for (const t of items) { const c = currencyOf(t.account_id); out[c] = (out[c] ?? 0) + Number(t.amount); }
    return out;
  }
  ...
  const totals = useMemo(() => {
    ...
    const currencies = sortCurrencies(Array.from(new Set([...Object.keys(income), ...Object.keys(expense), ...accounts.map((a) => a.currency || "BRL")])));
    const byCurrency = currencies.map((currency) => ({ currency, income: income[currency] ?? 0, expense: expense[currency] ?? 0, result: (income[currency] ?? 0) - (expense[currency] ?? 0) }));
    ...
    return { byCurrency, balance: balanceByAccount.reduce((s, a) => s + a.balanceBRL, 0), assetTotal, liabilityTotal };
  }, [transactions, assets, balanceByAccount, accounts, accountCurrency]);

  const chartDataByCurrency = useMemo(() => {
    const currencies = sortCurrencies(Array.from(new Set(accounts.map((a) => a.currency || "BRL"))));
    ...
    return currencies.map((currency) => ({
      ...
      data: Array.from({ length: 6 }, (_, index) => {
        ...
        const rows = transactions.filter((tx) => {
          const d = new Date(`${tx.transaction_date}T12:00:00`);
          return d.getMonth() === date.getMonth() && d.getFullYear() === date.getFullYear() && currencyOf(tx.account_id) === currency;
        });
  ...
  const centerSummary = useMemo(() => {
    const rows = costCenters.map((center) => {
      const own = transactions.filter((t) => t.cost_center_id === center.id && t.transaction_type !== "transfer");
      ...
      const currencies = sortCurrencies(Array.from(new Set([...Object.keys(income), ...Object.keys(expense)])));
    ...
    if (orphan.length) {
      ...
      const currencies = sortCurrencies(Array.from(new Set([...Object.keys(income), ...Object.keys(expense)])));
      rows.push({ id: "none", user_id: "", name: "Sem centro de custo", center_type: "other", description: null, color: "orange", is_active: true, created_at: "", updated_at: "", income, expense, currencies, count: orphan.length } as (typeof rows)[number]);
    }
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <Dialog open={modal !== null} onOpenChange={(openState) => !openState && setModal(null)}><DialogContent><DialogHeader><DialogTitle>{(editingId ? "Editar " : "Novo ") + (modal === "account" ? "conta" : modal === "category" ? "categoria" : modal === "asset" ? "item patrimonial" : modal === "cost_center" ? "centro de custo" : "lançamento")}</DialogTitle><DialogDescription>Preencha os dados para manter seus números atualizados.</DialogDescription></DialogHeader><form onSubmit={save} className="space-y-4">{modal === "account" && <><Field label="Nome"><Input required {...field("name")} placeholder="Conta principal" /></Field><Field label="Instituição"><Input {...field("institution")} placeholder="Nome do banco" /></Field><Field label="Tipo"><select className={selectClass} {...field("account_type")}><option value="checking">Conta corrente</option><option value="savings">Poupança</option><option value="cash">Dinheiro</option><option value="investment">Investimento</option><option value="credit">Cartão de crédito</option></select></Field><Field label="Moeda"><select className={selectClass} {...field("currency")}>{currencyOptions.map(c => <option key={c.code} value={c.code}>{c.label}</option>)}</select>{form.currency !== "BRL" && <p className="text-xs text-muted-foreground">Só o saldo atual desta conta é convertido para reais (cotação do dia anterior). Lançamentos continuam na moeda original.</p>}</Field><Field label="Saldo inicial"><Input required type="number" step="0.01" {...field("initial_balance")} /></Field></>}{modal === "category" && <><Field label="Nome"><Input required {...field("name")} /></Field><Field label="Tipo"><select className={selectClass} {...field("category_type")}><option value="expense">Despesa</option><option value="income">Receita</option></select></Field><Field label="Categoria superior (opcional)"><select className={selectClass} {...field("parent_id")}><option value="">Nenhuma</option>{categories.filter(c => c.category_type === form.category_type).map(c => <option key={c.id} value={c.id}>{categoryPath(c.id)}</option>)}</select></Field></>}{modal === "asset" && <><Field label="Nome"><Input required {...field("name")} placeholder="Apartamento, veículo, financiamento…" /></Field><Field label="Natureza"><select className={selectClass} {...field("asset_type")}><option value="asset">Ativo</option><option value="liability">Passivo</option></select></Field><Field label="Classe"><Input required {...field("asset_class")} placeholder="Imóvel, veículo, dívida…" /></Field><Field label="Valor atual"><Input required type="number" min="0" step="0.01" {...field("value")} /></Field></>}{modal === "cost_center" && <><Field label="Nome"><Input required {...field("name")} placeholder="Studio 01, Apto Airbnb, Pessoal…" /></Field><Field label="Tipo"><select className={selectClass} {...field("center_type")}><option value="property">Imóvel</option><option value="business">Negócio</option><option value="personal">Pessoal</option><option value="other">Outro</option></select></Field><Field label="Descrição (opcional)"><Input {...field("description")} placeholder="Locação por temporada no Airbnb e Booking" /></Field></>}{modal === "transaction" && <><Field label="Tipo"><select className={selectClass} {...field("transaction_type")}><option value="expense">Despesa</option><option value="income">Receita</option><option value="transfer">Transferência</option></select></Field><Field label="Descrição"><Input required {...field("description")} /></Field><div className="grid grid-cols-2 gap-3"><Field label="Valor"><Input required type="number" min="0.01" step="0.01" {...field("amount")} /></Field><Field label="Data"><Input required type="date" {...field("transaction_date")} /></Field></div><Field label={form.transaction_type === "transfer" ? "Conta de origem" : "Conta"}><select required className={selectClass} {...field("account_id")}><option value="">Selecione</option>{accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select></Field>{form.transaction_type === "transfer" ? <Field label="Conta de destino"><select required className={selectClass} {...field("destination_account_id")}><option value="">Selecione</option>{accounts.filter(a => a.id !== form.account_id).map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select></Field> : <Field label="Categoria"><select className={selectClass} {...field("category_id")}><option value="">Sem categoria</option>{categories.filter(c => c.category_type === form.transaction_type).map(c => <option key={c.id} value={c.id}>{categoryPath(c.id)}</option>)}</select></Field>}<Field label="Centro de custo"><select className={selectClass} {...field("cost_center_id")}><option value="">Sem centro de custo</option>{costCenters.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>{!costCenters.length && <p className="text-xs text-muted-foreground">Cadastre um centro de custo para separar por imóvel, plataforma ou área pessoal.</p>}</Field></>}{error && <p className="rounded-md bg-destructive-soft p-3 text-sm text-destructive">{error}</p>}<div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setModal(null)}>Cancelar</Button><Button type="submit" disabled={saving}>{saving ? "Salvando…" : "Salvar"}</Button></div></form></DialogContent></Dialog>
  </div>;
}

function RowActions({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) { return <div className="flex items-center gap-1"><Button variant="ghost" size="icon" aria-label="Editar" onClick={onEdit}><Pencil className="size-4" /></Button><Button variant="ghost" size="icon" aria-label="Excluir" className="text-expense" onClick={onDelete}><Trash2 className="size-4" /></Button></div>; }
...
function Metric({ label, value, tone, currency = "BRL" }: { label: string; value: number; tone?: "positive" | "negative"; currency?: string }) { return <div className="rounded-lg border border-border bg-card p-5"><p className="text-xs font-medium uppercase text-muted-foreground">{label}</p><p className={cn("mt-2 font-mono text-2xl font-medium tabular-nums", tone === "positive" && "text-income", tone === "negative" && "text-expense")}>{formatCurrency(value, currency)}</p></div>; }
function Dashboard({ totals, chartDataByCurrency, transactions, accounts, categoryPath, centerName, centerSummary, onEditTx, onDeleteTx }: any) {
  return <div className="space-y-4">
    ...
    {chartDataByCurrency.map((cd: any) => <div key={cd.currency} className="grid gap-4 xl:grid-cols-5">
      <section className="rounded-lg border border-border bg-card p-5 xl:col-span-3">
        <div><h2 className="font-semibold">Receita × despesa{cd.currency !== "BRL" ? ` — ${cd.currency}` : ""}</h2><p className="text-xs text-muted-foreground">Últimos seis meses</p></div>
        <div className="mt-4 h-64"><ResponsiveContainer width="100%" height="100%"><BarChart data={cd.data}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--border)" /><XAxis dataKey="month" tickLine={false} axisLine={false} /><YAxis tickLine={false} axisLine={false} width={55} /><Tooltip formatter={(v) => formatCurrency(Number(v), cd.currency)} /><Bar dataKey="receita" fill="var(--income)" radius={[3, 3, 0, 0]} /><Bar dataKey="despesa" fill="var(--expense)" radius={[3, 3, 0, 0]} /></BarChart></ResponsiveContainer></div>
      </section>
      <section className="rounded-lg border border-border bg-card p-5 xl:col-span-2">
        <h2 className="font-semibold">Fluxo de caixa{cd.currency !== "BRL" ? ` — ${cd.currency}` : ""}</h2><p className="text-xs text-muted-foreground">Resultado mensal</p>
        <div className="mt-4 h-64"><ResponsiveContainer width="100%" height="100%"><AreaChart data={cd.data.map((d: any) => ({ ...d, fluxo: d.receita - d.despesa }))}><defs><linearGradient id={`cash-${cd.currency}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--primary)" stopOpacity={.32} /><stop offset="100%" stopColor="var(--primary)" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--border)" /><XAxis dataKey="month" tickLine={false} axisLine={false} /><YAxis hide /><Tooltip formatter={(v) => formatCurrency(Number(v), cd.currency)} /><Area type="monotone" dataKey="fluxo" stroke="var(--primary)" strokeWidth={2} fill={`url(#cash-${cd.currency})`} /></AreaChart></ResponsiveContainer></div>
      </section>
    </div>)}
    <section className="rounded-lg border border-border bg-card"><div className="border-b border-border px-5 py-4"><h2 className="font-semibold">Movimentações recentes</h2></div><TransactionRows transactions={transactions.slice(0, 6)} accounts={accounts} categoryPath={categoryPath} centerName={centerName} onEditTx={onEditTx} onDeleteTx={onDeleteTx} /></section>
    {centerSummary.length > 0 && <section className="rounded-lg border border-border bg-card">
      <div className="border-b border-border px-5 py-4"><h2 className="font-semibold">Resultado por centro de custo</h2><p className="text-xs text-muted-foreground">Receitas e despesas alocadas, por moeda</p></div>
      <div className="divide-y divide-border">{centerSummary.map((r: any) => <div key={r.id} className="px-5 py-3">
        <p className="text-sm font-medium">{r.name}</p>
        <div className="mt-1 space-y-1">{r.currencies.length ? r.currencies.map((c: string) => <div key={c} className="grid grid-cols-2 items-center gap-3 text-xs sm:grid-cols-4">
          <span className="text-muted-foreground">{c}</span>
          <span className="hidden font-mono tabular-nums text-income sm:block">{formatCurrency(r.income[c] ?? 0, c)}</span>
          <span className="hidden font-mono tabular-nums text-expense sm:block">{formatCurrency(r.expense[c] ?? 0, c)}</span>
          <span className={cn("text-right font-mono tabular-nums", (r.income[c] ?? 0) - (r.expense[c] ?? 0) >= 0 ? "text-income" : "text-expense")}>{formatCurrency((r.income[c] ?? 0) - (r.expense[c] ?? 0), c)}</span>
        </div>) : <p className="text-xs text-muted-foreground">Sem lançamentos</p>}</div>
      </div>)}</div>
    </section>}
  </div>;
}
function Accounts({ accounts, onAdd, rateDate, onEdit, onDelete }: { accounts: (Account & { balance: number; currency: string; balanceBRL: number })[]; onAdd: () => void; rateDate: string; onEdit: (a: Account) => void; onDelete: (a: Account) => void }) { if (!accounts.length) return <Empty title="Comece pelas suas contas" text="Cadastre bancos, carteiras e investimentos." onAdd={onAdd}/>; return <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{accounts.map(a => <div key={a.id} className="rounded-lg border border-border bg-card p-5"><div className="flex items-start justify-between"><div className="grid size-10 place-items-center rounded-md bg-primary-soft text-primary"><Landmark /></div><div className="flex items-center gap-1"><span className="rounded-full bg-muted px-2 py-1 text-xs text-muted-foreground">{a.is_active ? "Ativa" : "Inativa"}</span><RowActions onEdit={() => onEdit(a)} onDelete={() => onDelete(a)} /></div></div><h2 className="mt-5 font-semibold">{a.name}</h2><p className="text-sm text-muted-foreground">{a.institution || "Conta pessoal"}</p><p className="mt-4 font-mono text-2xl tabular-nums">{formatCurrency(a.balance, a.currency)}</p>{a.currency !== "BRL" && <p className="mt-1 text-xs text-muted-foreground">{money.format(a.balanceBRL)} pela cotação{rateDate ? ` de ${dateFmt.format(new Date(`${rateDate}T12:00:00`))}` : " do dia anterior"}</p>}</div>)}</div>; }
function TransactionRows({ transactions, accounts, categoryPath, centerName, onEditTx, onDeleteTx }: { transactions: Transaction[]; accounts: Account[]; categoryPath: (id:string|null)=>string; centerName: (id:string|null)=>string; onEditTx?: (tx: Transaction)=>void; onDeleteTx?: (tx: Transaction)=>void }) { return <div className="divide-y divide-border">{transactions.map(tx => { const positive=tx.transaction_type==="income"; const account = accounts.find(a=>a.id===tx.account_id); const currency = account?.currency || "BRL"; return <div key={tx.id} className="grid grid-cols-[1fr_auto] items-center gap-4 px-5 py-3 transition-colors hover:bg-muted/40 sm:grid-cols-[110px_1fr_1fr_auto_auto]"><span className="hidden text-xs text-muted-foreground sm:block">{dateFmt.format(new Date(`${tx.transaction_date}T12:00:00`))}</span><div><p className="text-sm font-medium">{tx.description}</p><p className="text-xs text-muted-foreground sm:hidden">{dateFmt.format(new Date(`${tx.transaction_date}T12:00:00`))}</p></div><div className="hidden text-xs text-muted-foreground sm:block">{tx.transaction_type === "transfer" ? "Transferência" : categoryPath(tx.category_id)} · {account?.name}{tx.transaction_type !== "transfer" && <span className="ml-1 rounded-full bg-primary-soft px-2 py-0.5 text-primary">{centerName(tx.cost_center_id)}</span>}</div><span className={cn("font-mono text-sm tabular-nums", positive ? "text-income" : tx.transaction_type === "expense" ? "text-expense" : "text-foreground")}>{positive ? "+" : tx.transaction_type === "expense" ? "−" : ""}{formatCurrency(Number(tx.amount), currency)}</span>{onEditTx && onDeleteTx && <RowActions onEdit={() => onEditTx(tx)} onDelete={() => onDeleteTx(tx)} />}</div>})}{!transactions.length && <p className="p-8 text-center text-sm text-muted-foreground">Nenhum lançamento neste período.</p>}</div>; }
function Transactions({ transactions, accounts, categoryPath, centerName, onAdd, onEditTx, onDeleteTx }: any) { if (!transactions.length) return <Empty title="Registre a primeira movimentação" text="Adicione receitas, despesas ou transferências." onAdd={onAdd}/>; return <section className="rounded-lg border border-border bg-card"><TransactionRows transactions={transactions} accounts={accounts} categoryPath={categoryPath} centerName={centerName} onEditTx={onEditTx} onDeleteTx={onDeleteTx}/></section>; }
function CostCenters({ rows, onAdd, onEdit, onDelete }: any) {
  if (!rows.length) return <Empty title="Separe por centro de custo" text="Crie centros como Studio 01, Apto Airbnb ou Pessoal e saiba de onde vem e para onde vai cada valor." onAdd={onAdd}/>;
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{rows.map((r:any)=><div key={r.id} className="rounded-lg border border-border bg-card p-5">
      <div className="flex items-start justify-between"><div className="grid size-10 place-items-center rounded-md bg-primary-soft text-primary"><Building2/></div><div className="flex items-center gap-1"><span className="rounded-full bg-muted px-2 py-1 text-xs text-muted-foreground">{centerTypeLabel[r.center_type] ?? "Outro"}</span>{r.id !== "none" && <RowActions onEdit={() => onEdit(r)} onDelete={() => onDelete(r)} />}</div></div>
      ...
      <div className="mt-4 space-y-2 text-sm">{r.currencies.length ? r.currencies.map((c: string) => <dl key={c} className="space-y-1 border-t border-border pt-2 first:border-0 first:pt-0">
        {r.currencies.length > 1 && <p className="text-xs font-medium text-muted-foreground">{c}</p>}
        <div className="flex justify-between"><dt className="text-muted-foreground">Receitas</dt><dd className="font-mono tabular-nums text-income">{formatCurrency(r.income[c] ?? 0, c)}</dd></div>
        <div className="flex justify-between"><dt className="text-muted-foreground">Despesas</dt><dd className="font-mono tabular-nums text-expense">{formatCurrency(r.expense[c] ?? 0, c)}</dd></div>
        <div className="flex justify-between border-t border-border pt-1"><dt className="text-muted-foreground">Resultado</dt><dd className={cn("font-mono tabular-nums", (r.income[c] ?? 0)-(r.expense[c] ?? 0)>=0?"text-income":"text-expense")}>{formatCurrency((r.income[c] ?? 0)-(r.expense[c] ?? 0), c)}</dd></div>
      </dl>) : <p className="text-xs text-muted-foreground">Sem lançamentos</p>}</div>
    </div>)}</div>
  </div>;
}
function Categories({ categories, onAdd, onEdit, onDelete }: { categories: Category[]; onAdd:()=>void; onEdit:(c:Category)=>void; onDelete:(c:Category)=>void }) { if(!categories.length) return <Empty title="Organize seus lançamentos" text="Crie grupos e subgrupos sem limite de níveis." onAdd={onAdd}/>; const roots=categories.filter(c=>!c.parent_id); const Branch=({c,depth=0}:{c:Category;depth?:number})=><><div className="flex items-center justify-between border-b border-border px-4 py-3"><span className="flex items-center gap-2 text-sm">{Array.from({length:depth}).map((_,index)=><span key={index} className="w-3"/>)}<ChevronRight className="size-4 text-muted-foreground"/>{c.name}</span><span className={cn("rounded-full px-2 py-1 text-xs", c.category_type==="income"?"bg-income-soft text-income":"bg-expense-soft text-expense")}>{c.category_type==="income"?"Receita":"Despesa"}</span><RowActions onEdit={() => onEdit(c)} onDelete={() => onDelete(c)} /></div>{categories.filter(x=>x.parent_id===c.id).map(child=><Branch key={child.id} c={child} depth={depth+1}/>)}</>; return <div className="overflow-hidden rounded-lg border border-border bg-card">{roots.map(c=><Branch key={c.id} c={c}/>)}</div>; }
function Assets({ assets, totals, onAdd, onEdit, onDelete }: any) { return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><Metric label="Ativos" value={totals.assetTotal} tone="positive"/><Metric label="Passivos" value={totals.liabilityTotal} tone="negative"/><Metric label="Patrimônio líquido" value={totals.assetTotal-totals.liabilityTotal}/></div>{assets.length ? <div className="grid gap-3 sm:grid-cols-2">{assets.map((a:Asset)=><div key={a.id} className="flex items-center gap-3 rounded-lg border border-border bg-card p-5"><div><p className="font-semibold">{a.name}</p><p className="text-xs text-muted-foreground">{a.asset_class}</p></div><div className="ml-auto text-right"><p className={cn("font-mono tabular-nums",a.asset_type==="asset"?"text-income":"text-expense")}>{money.format(Number(a.value))}</p><p className="text-xs text-muted-foreground">{a.asset_type==="asset"?"Ativo":"Passivo"}</p></div><RowActions onEdit={() => onEdit(a)} onDelete={() => onDelete(a)} /></div>)}</div> : <Empty title="Monte seu balanço patrimonial" text="Cadastre seus bens, investimentos e dívidas." onAdd={onAdd}/>}</div>; }

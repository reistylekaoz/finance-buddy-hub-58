  ...
  return <div className="min-h-screen bg-background text-foreground">
    <div className="flex min-h-screen">
      <aside className={cn("fixed inset-y-0 left-0 z-40 w-64 border-r border-border bg-sidebar p-4 transition-transform md:sticky md:translate-x-0", mobileOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex items-center justify-between px-2 py-2"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-md bg-primary font-display text-lg font-semibold text-primary-foreground">F</div><div><p className="font-display text-lg font-semibold">Fluxora</p><p className="text-xs text-muted-foreground">Finanças pessoais</p></div></div><Button variant="ghost" size="icon" className="md:hidden" onClick={() => setMobileOpen(false)} aria-label="Fechar menu"><X /></Button></div>
...
function TransactionRows({ transactions, accounts, categoryPath, centerName }: { transactions: Transaction[]; accounts: Account[]; categoryPath: (id:string|null)=>string; centerName: (id:string|null)=>string }) { return <div className="divide-y divide-border">{transactions.map(tx => { const positive=tx.transaction_type==="income"; return <div key={tx.id} className="grid grid-cols-[1fr_auto] items-center gap-4 px-5 py-3 transition-colors hover:bg-muted/40 sm:grid-cols-[110px_1fr_1fr_auto]"><span className="hidden text-xs text-muted-foreground sm:block">{dateFmt.format(new Date(`${tx.transaction_date}T12:00:00`))}</span><div><p className="text-sm font-medium">{tx.description}</p><p className="text-xs text-muted-foreground sm:hidden">{dateFmt.format(new Date(`${tx.transaction_date}T12:00:00`))}</p></div><div className="hidden text-xs text-muted-foreground sm:block">{tx.transaction_type === "transfer" ? "Transferência" : categoryPath(tx.category_id)} · {accounts.find(a=>a.id===tx.account_id)?.name}{tx.transaction_type !== "transfer" && <span className="ml-1 rounded-full bg-primary-soft px-2 py-0.5 text-primary">{centerName(tx.cost_center_id)}</span>}</div><span className={cn("font-mono text-sm tabular-nums", positive ? "text-income" : tx.transaction_type === "expense" ? "text-expense" : "text-foreground")}>{positive ? "+" : tx.transaction_type === "expense" ? "−" : ""}{money.format(Number(tx.amount))}</span></div>})}{!transactions.length && <p className="p-8 text-center text-sm text-muted-foreground">Nenhum lançamento neste período.</p>}</div>; }
...
function CostCenters({ rows, onAdd }: any) { if (!rows.length) return <Empty title="Separe por centro de custo" text="Crie centros como Studio 01, Apto Airbnb ou Pessoal e saiba de onde vem e para onde vai cada valor." onAdd={onAdd}/>; return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><Metric label="Receitas alocadas" value={rows.reduce((s:number,r:any)=>s+r.income,0)} tone="positive"/><Metric label="Despesas alocadas" value={rows.reduce((s:number,r:any)=>s+r.expense,0)} tone="negative"/><Metric label="Resultado" value={rows.reduce((s:number,r:any)=>s+r.result,0)}/></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{rows.map((r:any)=><div key={r.id} className="rounded-lg border border-border bg-card p-5"><div className="flex items-start justify-between"><div className="grid size-10 place-items-center rounded-md bg-primary-soft text-primary"><Building2/></div><span className="rounded-full bg-muted px-2 py-1 text-xs text-muted-foreground">{centerTypeLabel[r.center_type] ?? "Outro"}</span></div><h2 className="mt-5 font-semibold">{r.name}</h2><p className="text-xs text-muted-foreground">{r.description || `${r.count} lançamento${r.count===1?"":"s"}`}</p><dl className="mt-4 space-y-1 text-sm"><div className="flex justify-between"><dt className="text-muted-foreground">Receitas</dt><dd className="font-mono tabular-nums text-income">{money.format(r.income)}</dd></div><div className="flex justify-between"><dt className="text-muted-foreground">Despesas</dt><dd className="font-mono tabular-nums text-expense">{money.format(r.expense)}</dd></div><div className="flex justify-between border-t border-border pt-1"><dt className="text-muted-foreground">Resultado</dt><dd className={cn("font-mono tabular-nums", r.result>=0?"text-income":"text-expense")}>{money.format(r.result)}</dd></div></dl></div>)}</div></div>; }
